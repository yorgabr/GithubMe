function New-RemoteRepository {
    ...
    if ($Provider -eq 'GitHub') {
        $apiUrl = if ($ApiBaseUrl) { $ApiBaseUrl } else { 'https://api.github.com' }
        ...
        try {
            ...
            $code = 0
            if ($_.Response) { $code = [int]$_.Response.StatusCode }
            elseif ($_.Exception.Response) { $code = [int]$_.Exception.Response.StatusCode }
            ...
            $detail = switch ($code) {
                ...
                default { "GitHub API error (${code}): $($_.Message)" }
            }
    ...
    elseif ($Provider -eq 'GitLab') {
        $apiUrl = if ($ApiBaseUrl) { $ApiBaseUrl } else { 'https://gitlab.com/api/v4' }
        ...
        try {
            ...
            $detail = switch ($code) {
                ...
                default { "GitLab API error (${code}): $($_.Message)" }
            }
            Write-GitMeLog -Level Error -Message $detail
            throw
        }
