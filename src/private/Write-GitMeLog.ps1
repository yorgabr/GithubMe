function Write-GitMeLog {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][ValidateSet('Info', 'Warn', 'Success', 'Error', 'Debug')][string]$Level,
    ...
    switch ($Level) {
        'Info' {
            if ($script:GitMeLogLevel -ne 'Quiet') {
        ...
        'Debug' {
            if ($script:GitMeLogLevel -eq 'Debug') {
                [Console]::Out.WriteLine("${Magenta}[DEBUG]${Reset} $Message")
            }
        }
    }
}
